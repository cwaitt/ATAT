...
class Lattice:
    ...
    def __init__(self, folder_path):
        ...
        for line in lat[4:]:
            ...
            self.sites = pd.concat([pd.DataFrame([site],columns=['a','b','c','atom']),self.sites],ignore_index=True) # new function
            #self.sites = self.sites.append(pd.DataFrame([site], columns=['a', 'b', 'c', 'atom'])) # old function
...
class Structure:
    ...
    def __init__(self, lattice, folder_path):
        ...
        self.m = [int(number) for number in str_req[0].split()]
        self.l = [int(number) for number in str_req[1].split()]
        self.n = [int(number) for number in str_req[2].split()]
        self.u_mult = int(np.mean(self.m)/np.mean(lattice.u))
        self.v_mult = int(np.mean(self.l)/np.mean(lattice.v))
        self.w_mult = int(np.mean(self.n)/np.mean(lattice.w))
        self.ncell = self.u_mult*self.v_mult*self.w_mult

        self.axes_abc = np.array((self.m, self.l, self.n))
    ...
    def prepare_str_out(self):
        ...
        with open(filepath, 'a') as str_file:
            ...
            str_file.write('{} {} {}\n'.format(float(self.l[0]),float(self.l[1]),float(self.l[2])))
            str_file.write('{} {} {}\n'.format(float(self.m[0]),float(self.m[1]),float(self.m[2])))
            str_file.write('{} {} {}\n'.format(float(self.n[0]),float(self.n[1]),float(self.n[2])))
    ...
    def random_config_swap(self, atom_num, penalty={}, prob={}, num_vecs=1, num_step=100, vis=0, process=0, ptfile=0):
        ...
        for i in range(num_vecs):
            ...
            while (step < num_step):
                ...
                s_index = random.sample(list(selected_sites[group]), 1)[0]
                us_index = random.sample(list(unselected_sites[group]), 1)[0]
